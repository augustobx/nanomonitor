...
docker run --rm \
  ...
  sh -c '
    ...
    go build \
      ...
      -trimpath \
      -ldflags "$LDFLAGS" \
      -o /src/.build-phase2/nanoagent.exe \
      ./cmd/nanoagent
    ...
    go build \
      ...
      -o /src/.build-phase2/nanotray.exe \
      ./cmd/nanotray
  '
...
\cp -f .build-phase2/nanoagent.exe installer/cmd/installer/embedded/nanoagent.exe
\cp -f .build-phase2/nanotray.exe installer/cmd/installer/embedded/nanotray.exe
...
docker run --rm \
  ...
  sh -c '
    ...
    go build \
      ...
      -trimpath \
      -ldflags "-s -w" \
      -o /src/.build-phase2/NanoMonitor-Setup.exe \
      ./cmd/installer
  '

echo "=== 4/7 PUBLISH DOWNLOAD ARTIFACTS ==="
mkdir -p downloads
\cp -f .build-phase2/NanoMonitor-Setup.exe downloads/NanoMonitor-Setup.exe
\cp -f .build-phase2/nanoagent.exe downloads/nanoagent.exe
\cp -f .build-phase2/nanotray.exe downloads/nanotray.exe
...
sha256sum \
  .build-phase2/nanoagent.exe \
  .build-phase2/nanotray.exe \
  .build-phase2/NanoMonitor-Setup.exe \
